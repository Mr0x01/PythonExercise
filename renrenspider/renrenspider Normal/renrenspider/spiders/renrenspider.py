...
class Renrenspider(scrapy.Spider):
    ...
    def login_page(self, response):
        lbskey = response.css(
            "div.form-inline>input[name=lbskey]::attr(value)").extract_first()  # 取出登陆需要的值
        email = input("输入账号:")  # 账号
    ...
    def home_page(self, response):
        ...
        return response.follow(
            url="http://3g.renren.com/friendlist.do?curpage=" + str(page),
            cookies=response.meta["cookies"],
            callback=self.frinds_page,
            meta={"cookies": response.meta["cookies"], "page": page+1}
        )
    ...
    def frinds_page(self, response):
        ...
        if len(friendlist) > 0:
            for friend in friendlist:
                ...
                friend_id = urllib.parse.parse_qs(
                    friend_link.split("?")[1])["id"][0]
                album_link = "http://3g.renren.com/album/wmyalbum.do?id={0}".format(
                    friend_id)
            ...
            yield scrapy.Request(
                url="http://3g.renren.com/friendlist.do?curpage=" + str(response.meta["page"]),
                cookies=response.meta["cookies"],
                callback=self.frinds_page,
                    meta={
                        "cookies": response.meta["cookies"], "page": response.meta["page"]+1}
            )
        # next_page_link = response.css(
        #     "div.l > a[title='下一页']::attr(href)").extract_first()
    ...
    def each_album(self, response):
        ...
        if next_page_link != None:
            yield scrapy.Request(
                ...
                callback=self.each_album,
                meta={
                    "cookies": response.meta["cookies"], "dont_redirect": True}
            )

    def each_img(self, response):
        logging.info("相片详情")
        img_item = RenrenspiderItem()
        friend_name = response.css(
            "body > div.ssec > b > a::text").extract_first()
        img_item["friend_name"] = friend_name
        album_name = response.css(
            "body > div:nth-child(14) > b::text").extract_first()
        img_item["album_name"] = album_name
        comment = ""
        try:
            comment = response.css(
                "body > div:nth-child(15) > p:nth-child(3)")[0].css("::text").extract()[1].strip()
        except IndexError:
            comment = response.css(
                "body > div:nth-child(15) > p:nth-child(3) *::text").extract_first().strip()
        img_item["img_comment"] = comment
        datetime = response.css(
            "body > div:nth-child(15) > p.time::text").extract_first()
